...
# create a map object and fill it with the critical positions
map = Map_Obj(1)
[start_node, stop_node, end_goal_pos, path_to_map] = map.fill_critical_positions(1)
map_array = map.read_map(path_to_map)
print(map_array)

# set the start and goal position in the map
...
def get_neighbors(node):
     ...
     #check if the neighbors are walkable
     walkable_neighbors = []
...
def a_star(start_node, stop_node):
    ...
    # making a string of the node to use it as a key in the dictionary
    def make_key(node):
          x=str(node[1])
          y=str(node[0])
          if(len(x)==1):
    ...
    # the dictionary that will contain the parents of the nodes
    parents = {}
    ...
    while len(open_list) > 0:
          ...
          # if the node is not found, the path doesn't exist
          if n == None:
